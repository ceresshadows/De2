...
def fuse_and_map(all_series, average_series_1, event_template_1, ids_to_check, feature, rpt_pen):
    ...
    # Assuming ids_to_plot contains more than 4 IDs and you want the first 4
    ids_to_plot = ids_to_check[:4]
    # Select the first four series based on the updated ids_to_plot
    ...
    # Calculate limits for x-axis and y-axis
    x_min = 0
    x_max = 80
    y_min = 1.8  # Assuming your events start from 0
    y_max = 2.5
    # Plot the time series of fusion and its events
    axes[0].plot(average_series_1, color='green', linewidth=2)  # Green thick line
    for event in event_template_1:
        axes[0].axvline(event, color='red', linestyle='--')
...
def plot_events(tmp, axis, signal, feature, result):
    ...
    for i in range(1, len(result)):
        ...
        annotation_text = f"Rate: {change_rate:.2f}\nMagnitude: {change_magnitude:.2f}\nMean: {mean:.2f}\nPosterior: {confidence:.2f}"
        axis.annotate(annotation_text, xy=(start, signal[start]),
                      xytext=(0, 10), textcoords='offset points', arrowprops=dict(arrowstyle="->"),
                      fontsize=12, ha='center')
        axis.axvline(x=start, color='red',linestyle='--', alpha=0.5)  # Draw a vertical line on the start dot
        axis.axvline(x=end,color='red', linestyle='--', alpha=0.5)    # Draw a vertical line at the end
...
feature_list = [('NPC1Theta', 3, 15)] #1.5, 8; 5, 20
